module.exports = class Peer {
  ...
  async createProducer(producerTransportId, rtpParameters, kind,appData) {
    ...
    console.log("检查编码==》" + JSON.stringify(producer));
    console.log("3放入生产者！");
  ...
  async createConsumer(consumer_transport_id, producer_id, rtpCapabilities,_appData) {
    let consumerTransport = this.transports.get(consumer_transport_id);

    if (!consumerTransport) {
      console.error('Consumer transport not found:', consumer_transport_id);
      return;
    }
    ...
    try {
        consumer = await consumerTransport.consume({
            ...
            rtpCapabilities,
            paused: false,
            appData:  _appData// 关键修复：传递生产者的 appData
        });
    } catch (error) {
